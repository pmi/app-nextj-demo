    ...
    extends BaseContextHandler
{
    ...
    private Context createInitContext()
    {
        ...
        return ContextBuilder.from( ContextAccessor.current() ).authInfo(
            AuthenticationInfo.create().principals( RoleKeys.CONTENT_MANAGER_ADMIN ).user( User.ANONYMOUS ).build() ).branch(
            ContentConstants.BRANCH_DRAFT ).repositoryId( ProjectName.from( this.projectName ).getRepoId() ).build();
    }
    ...
    private void doInitialize()
    {
        ...
        final NodeImportResult nodeImportResult = this.exportService.get().importNodes(
            ImportNodesParams.create().source( source ).targetNodePath( NodePath.create( "/content" ).build() ).includeNodeIds(
                true ).includePermissions( false ).dryRun( false ).xslt( xsltTransformer ).xsltParam( "applicationId",
                                                                                                      appKey.toString() ).build() );
